...
class PickGame extends Component {
  ...
  init(values: any) {
    ...
    for (let i = 0; i < heigth; i++) {
      ...
      for (let j = 0; j < width; j++) {
        //生成图片索引
        let image_source_index = Math.floor(
          Math.random() * image_source.length,
        );
  ...
  imageOnClick = (index: number, e: any) => {
    ...
    if (
      pre_image != undefined &&
      pre_image.image_index == source[i][j].image_index &&
      pre_image.index != source[i][j].index
    ) {
          ...
          <div className="form-div">
            <Card>
              <Form
                ...
                autoComplete="off"
              >
                <Form.Item label="宽" name="width" initialValue={4}>
                  <InputNumber min={1} max={8} />
                </Form.Item>

                <Form.Item label="高" name="heigth" initialValue={4}>
                  <InputNumber min={1} max={6} />
                </Form.Item>
                <Form.Item label="时间" name="timeset" initialValue={100}>
